...
void dac_dma_wave_init(uint8_t outx, DMA_Stream_TypeDef *DMA_Streamx, uint8_t ch, uint32_t par, uint32_t mar);  /* DAC DMA输出波形初始化函数 */
void dac_dma_wave_enable(DMA_Stream_TypeDef *DMA_Streamx, uint16_t ndtr, uint16_t arr, uint16_t psc);           /* DAC DMA输出波形使能 */

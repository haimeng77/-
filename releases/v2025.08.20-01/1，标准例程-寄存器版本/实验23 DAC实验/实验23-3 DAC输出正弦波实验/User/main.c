...
{
    dac_dma_wave_enable(DMA2_Stream6, 100, arr, psc);
}
...
int main(void)
{
    ...
    /* 初始化DAC通道1 DMA波形输出 */
    dac_dma_wave_init(1, DMA2_Stream6, 67, (uint32_t)&DAC1->DHR12R1, (uint32_t)g_dac_sin_buf);  
    ...
    dac_dma_wave_enable(DMA2_Stream6, 100, 100 - 1, 20 - 1);/* 100Khz触发频率, 100个点, 可以得到最高1KHz的正弦波. */
    ...
    while (1)
    {
        ...
        {
            dac_creat_sin_buf(2048, 100);
            dac_dma_wave_enable(DMA2_Stream6, 100, 20 - 1, 20 - 1); /* 500Khz触发频率, 100个点, 得到最高5KHz的正弦波. */
        }
        else if (key == KEY1_PRES)  /* 低采样率 , 约1Khz波形 */
        {
            dac_creat_sin_buf(2048, 10);
            dac_dma_wave_enable(DMA2_Stream6, 10, 20 - 1, 20 - 1);  /* 500Khz触发频率, 10个点, 可以得到最高50KHz的正弦波. */
        } 

...
#define jpeg_buf_size   1 * 1024 * 1024 /* 定义JPEG数据缓存jpeg_buf的大小(4MB字节) */
...
uint32_t g_jpeg_data_buf[jpeg_buf_size]  __attribute__((section(".bss.ARM.__at_0XC01F4000")));  /* JPEG数据缓存buf,定义在LCD帧缓存之后 */
...
void rgb565_test(void)
{
    ...
    if (lcdltdc.pwidth != 0)        /* RGB屏 */
    {
        ...
        dcmi_dma_init((uint32_t)g_dcmi_line_buf[0], (uint32_t)g_dcmi_line_buf[1], lcddev.width / 2, 1, 1);
        
        BTIM_TIMX_INT->CR1 &= ~(0x01);  /* 关闭定时器,关闭帧率统计，打开的话，RGB屏，在串口打印的时候，会抖 */
    }
    else                                                                    /* MCU 屏 */
    {
...
int main(void)
{
    ...
    btim_timx_int_init(10000 - 1, 20000 - 1);

...
#define OLED_SPI_RST_CLK_ENABLE()       do{ RCC->AHB4ENR |= 1 << 0; }while(0)       /* PA口时钟使能 */

#define OLED_SPI_CS_PORT                GPIOB
#define OLED_SPI_CS_PIN                 SYS_GPIO_PIN7
#define OLED_SPI_CS_CLK_ENABLE()        do{ RCC->AHB4ENR |= 1 << 1; }while(0)       /* PB口时钟使能 */

#define OLED_SPI_RS_PORT                GPIOB
#define OLED_SPI_RS_PIN                 SYS_GPIO_PIN4
#define OLED_SPI_RS_CLK_ENABLE()        do{ RCC->AHB4ENR |= 1 << 3; }while(0)       /* PD口时钟使能 */

#define OLED_SPI_SCLK_PORT              GPIOC
#define OLED_SPI_SCLK_PIN               SYS_GPIO_PIN6
#define OLED_SPI_SCLK_CLK_ENABLE()      do{ RCC->AHB4ENR |= 1 << 2; }while(0)       /* PC口时钟使能 */

#define OLED_SPI_SDIN_PORT              GPIOC
#define OLED_SPI_SDIN_PIN               SYS_GPIO_PIN7
#define OLED_SPI_SDIN_CLK_ENABLE()      do{ RCC->AHB4ENR |= 1 << 2; }while(0)       /* PC口时钟使能 */
...
#define OLED_WR(x)      sys_gpio_pin_set(GPIOH, SYS_GPIO_PIN8, x)   /* 设置WR引脚 */
#define OLED_RD(x)      sys_gpio_pin_set(GPIOB, SYS_GPIO_PIN3, x)   /* 设置RD引脚 */

    ...
    { 
        ...
        if (res == 0)                                   /* 解析文件成功 */
        {
            if (wavctrl.bps == 16)
            {
                ...
                sai1_tx_dma_init(g_audiodev.saibuf1, g_audiodev.saibuf2, WAV_SAI_TX_DMA_BUFSIZE / 2, 1); /* 配置TX DMA,16位 */
            }
            else if (wavctrl.bps == 24)
            {
                es8388_sai_cfg(0, 0);                                                                    /* 飞利浦标准,24位数据长度 */
                sai1_saia_init(SAI_MODEMASTER_TX, SAI_CLOCKSTROBING_RISINGEDGE, SAI_DATASIZE_24);
                sai1_tx_dma_init(g_audiodev.saibuf1, g_audiodev.saibuf2, WAV_SAI_TX_DMA_BUFSIZE / 4, 2); /* 配置TX DMA */
             }
            ...
            if (res == 0)
            {
                ...
                { 
                    ...
                    if (wavwitchbuf)
                    {
                        fillnum = wav_buffill(g_audiodev.saibuf2, WAV_SAI_TX_DMA_BUFSIZE, wavctrl.bps);   /* 填充buf2 */
                    }
                    else
                    {
                        fillnum = wav_buffill(g_audiodev.saibuf1, WAV_SAI_TX_DMA_BUFSIZE, wavctrl.bps);   /* 填充buf1 */
                    }

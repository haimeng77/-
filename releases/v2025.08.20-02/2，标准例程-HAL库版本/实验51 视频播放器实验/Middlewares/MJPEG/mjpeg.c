...
uint8_t mjpeg_init(uint16_t offx, uint16_t offy, uint32_t width, uint32_t height)
{
    ...
    if (lcdltdc.pwidth == 0 || lcddev.dir == 0)     /* 如果不是RGB横屏,则需要p_rgb565buf数组 */
    {
 ...
 * @param       并填充到对应的GRAM里面,所有操作全硬件完成,速度非常快.
 * @param       sx,sy:起始坐标
...
uint8_t mjpeg_decode(uint8_t *buf, uint32_t bsize)
{
    ...
    while (1)
    {
        if (mjpeg.outbuf[mjpeg.outbuf_read_ptr].sta == 1)   /* buf里面有数据要处理 */
        {
            ...
            if (lcdltdc.pwidth == 0 || lcddev.dir == 0)     /* 非RGB横屏,需要先将YUV数据解码到p_rgb565buf,然后再从p_rgb565buf拷贝到LCD的GRAM */
            {

...
static TIM_IC_InitTypeDef g_timx_ic_cap_chy_handle;
...
static void tpad_reset(void)
{
    ...
    gpio_init_struct.Mode = GPIO_MODE_OUTPUT_PP;                        /* 复用推挽 */
    ...
    gpio_init_struct.Pin = TPAD_GPIO_PIN;                               /* 输入捕获的GPIO口 */
    gpio_init_struct.Mode = GPIO_MODE_AF_PP;                            /* 复用模式 */
...
static void tpad_timx_cap_init(uint32_t arr, uint16_t psc)
{
    ...
    HAL_GPIO_Init(TPAD_GPIO_PORT, &gpio_init_struct);                                                  /* TPAD引脚初始化 */

    g_timx_cap_chy_handle.Instance = TPAD_TIMX_CAP;                                                    /* 定时器2 */
    ...
    g_timx_cap_chy_handle.Init.ClockDivision = TIM_CLOCKDIVISION_DIV1;                                 /* 时钟分频因子，不分频 */
    HAL_TIM_IC_Init(&g_timx_cap_chy_handle);

    g_timx_ic_cap_chy_handle.ICPolarity = TIM_ICPOLARITY_RISING;                                       /* 上升沿捕获 */
    g_timx_ic_cap_chy_handle.ICSelection = TIM_ICSELECTION_DIRECTTI;                                   /* 映射到TI1上 */
    g_timx_ic_cap_chy_handle.ICPrescaler = TIM_ICPSC_DIV1;                                             /* 配置输入分频，不分频 */
    g_timx_ic_cap_chy_handle.ICFilter = 0;                                                             /* 配置输入滤波器，不滤波 */
    HAL_TIM_IC_ConfigChannel(&g_timx_cap_chy_handle, &g_timx_ic_cap_chy_handle, TPAD_TIMX_CAP_CHY);    /* 配置TIM2通道1 */

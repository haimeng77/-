...
TIM_HandleTypeDef g_tim_handle;      /* 定时器1句柄 */
...
void remote_init(void)
{
    ...
    g_tim_handle.Instance = REMOTE_IN_TIMX;                                             /* 通用定时器1 */
    g_tim_handle.Init.Prescaler = 200 - 1;                                              /* 预分频器,1M的计数频率,1us加1 */
    g_tim_handle.Init.CounterMode = TIM_COUNTERMODE_UP;                                 /* 向上计数器 */
...
void HAL_TIM_IC_MspInit(TIM_HandleTypeDef *htim)
{
    if (htim->Instance == REMOTE_IN_TIMX)
    {
        ...
        HAL_NVIC_SetPriority(REMOTE_IN_TIMX_IRQn, 1, 2);        /* 设置中断优先级，抢占优先级1，子优先级2 */
        HAL_NVIC_EnableIRQ(REMOTE_IN_TIMX_IRQn);                /* 开启ITM1中断 */
        
        /* 如果是通用定时器 ,不需要设置CC中断 */
        HAL_NVIC_SetPriority(REMOTE_IN_TIMX_CC_IRQn, 1, 3);     /* 设置中断优先级，抢占优先级1，子优先级3 */
...
void HAL_TIM_IC_CaptureCallback(TIM_HandleTypeDef *htim)
{
    if (htim->Instance == REMOTE_IN_TIMX)
    {
        ...
        if (RDATA)                                                                                                  /* 上升沿捕获 */
        {
            __HAL_TIM_SET_CAPTUREPOLARITY(&g_tim_handle, REMOTE_IN_TIMX_CHY, TIM_INPUTCHANNELPOLARITY_FALLING);     /* CC4P=1 设置为下降沿捕获 */
        ...
        else                                                                                                        /* 下降沿捕获 */
        {
            ...
            __HAL_TIM_SET_CAPTUREPOLARITY(&g_tim_handle, REMOTE_IN_TIMX_CHY, TIM_INPUTCHANNELPOLARITY_RISING);      /* 配置TIM通道1上升沿捕获 */
            ...
            if (g_remote_sta & 0x10)                        /* 完成一次高电平捕获 */
            {
                if (g_remote_sta & 0x80)                    /* 接收到了引导码 */
                {
                    ...
                    else if (dval > 2200 && dval < 2600)    /* 得到按键键值增加的信息 2500为标准值2.25ms */
                    {

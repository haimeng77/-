...
int main(void)
{
    ...
    lcd_init();                              /* 初始化LCD */
    adc_init();                              /* 初始化ADC */
    pwmdac_init(256 - 1, 1);                 /* PWM DAC初始化, Fpwm = 200MHz / 256 =781.25Khz */
    ...
    __HAL_TIM_SET_COMPARE(&g_tim15_handle, PWMDAC_TIMX_CHY, pwmval);            /* 输出新的PWM占空比 */
    ...
    while (1)
    {
        ...
        {
            adcx = __HAL_TIM_GET_COMPARE(&g_tim15_handle, PWMDAC_TIMX_CHY);     /* PWM DAC 定时器输出比较值 */
            ...
            adcx = adc_get_result_average(ADC_ADCX_CHY, 10);                    /* 得到ADC1通道19的转换结果 */

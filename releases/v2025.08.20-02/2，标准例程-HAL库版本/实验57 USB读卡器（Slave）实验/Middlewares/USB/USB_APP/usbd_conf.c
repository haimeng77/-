...
void HAL_PCD_MspInit(PCD_HandleTypeDef *hpcd)
{
    ...
    if (hpcd->Instance == USB2_OTG_FS)
    {
        ...
        gpio_init_struct.Alternate = GPIO_AF10_OTG1_FS;      /* 复用为OTG FS */
        ...
        HAL_GPIO_WritePin(GPIOG, GPIO_PIN_13, GPIO_PIN_SET); /* 开启USB HOST电源供电C */ 
        
        HAL_NVIC_SetPriority(OTG_FS_IRQn, 1, 0);
        HAL_NVIC_EnableIRQ(OTG_FS_IRQn);                     /* 使能 OTG FS 中断 */
    }
...
USBD_StatusTypeDef USBD_LL_Init(USBD_HandleTypeDef *pdev)
{
    if (pdev->id == DEVICE_FS)
    {
        ...
        g_pcd_usb_otg_fs.Instance = USB2_OTG_FS;                /* 使用USB OTG FS */
        g_pcd_usb_otg_fs.Init.dev_endpoints = 8;                /* 端点数为8 */
        g_pcd_usb_otg_fs.Init.speed = PCD_SPEED_FULL;           /* USB全速(12Mbps) */
        g_pcd_usb_otg_fs.Init.ep0_mps = 0X40;                   /* 设置端点0的最大包大小为0X40(64字节) */
        g_pcd_usb_otg_fs.Init.phy_itface = PCD_PHY_EMBEDDED;    /* 使用内部PHY */
        g_pcd_usb_otg_fs.Init.Sof_enable = 0;                   /* 使能SOF中断 */
        g_pcd_usb_otg_fs.Init.low_power_enable = DISABLE;       /* 不使能低功耗模式 */
        g_pcd_usb_otg_fs.Init.lpm_enable = 0;                   /* 使能连接电源管理 */
        g_pcd_usb_otg_fs.Init.vbus_sensing_enable = DISABLE;    /* 不使能VBUS检测 */
        g_pcd_usb_otg_fs.Init.use_dedicated_ep1 = 0;            /* 禁止EP1 dedicated中断 */

        g_pcd_usb_otg_fs.pData = pdev;                          /* hpcd的pData指向pdev */
        pdev->pData = &g_pcd_usb_otg_fs;                        /* pdev的pData指向hpcd */

...
void atim_timx_npwm_chy_set(uint32_t npwm)
{
    ...
    __HAL_TIM_ENABLE(&g_timx_npwm_chy_handle);                                      /* 使能定时器x */
}
...
void atim_timx_cplm_pwm_init(uint16_t arr, uint16_t psc)
{
    ...
    ATIM_TIMX_CPLM_CHYN_GPIO_CLK_ENABLE();                              /* 通道y互补通道对应IO口时钟使能 */
    ATIM_TIMX_CPLM_BKIN_GPIO_CLK_ENABLE();                              /* 通道y刹车输入对应IO口时钟使能 */
  
    gpio_init_struct.Pin = ATIM_TIMX_CPLM_BKIN_GPIO_PIN;                /* 刹车引脚 */
    gpio_init_struct.Mode = GPIO_MODE_AF_PP;                            /* 复用推挽 */
    gpio_init_struct.Pull = GPIO_PULLUP;                                /* 上拉 */
    gpio_init_struct.Speed = GPIO_SPEED_FREQ_HIGH;                      /* 高速 */
    gpio_init_struct.Alternate = ATIM_TIMX_CPLM_BKIN_GPIO_AF;           /* 端口复用 */
    HAL_GPIO_Init(ATIM_TIMX_CPLM_BKIN_GPIO_PORT, &gpio_init_struct);    /* 初始化IO */

    gpio_init_struct.Pin = ATIM_TIMX_CPLM_CHY_GPIO_PIN;                 /* 主通道引脚 */
    ...
    g_timx_cplm_pwm_handle.Init.ClockDivision = TIM_CLOCKDIVISION_DIV4;             /* CKD[1:0] = 10, tDTS = 4 * tCK_INT = Ft / 4 = 50Mhz*/
 ...
 * @note        tDTS = 1 / (Ft /  CKD[1:0]) = 1 / 50M = 20ns
 * @retval      无
 */

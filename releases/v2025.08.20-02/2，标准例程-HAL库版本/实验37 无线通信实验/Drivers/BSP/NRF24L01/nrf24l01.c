...
{
    __HAL_SPI_DISABLE(&g_spi2_handle);                       /* 先关闭SPI2 */
    g_spi2_handle.Init.CLKPolarity = SPI_POLARITY_LOW;       /* 串行同步时钟的空闲状态为低电平 */
    g_spi2_handle.Init.CLKPhase = SPI_PHASE_1EDGE;           /* 串行同步时钟的第1个跳变沿（上升或下降）数据被采样 */
...
uint8_t nrf24l01_check(void)
{
    ...
    uint8_t i;
    spi2_set_speed(SPI_BAUDRATEPRESCALER_32);                 /* spi速度为6.25Mhz（24L01的最大SPI时钟为10Mhz） */

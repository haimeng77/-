...
uint8_t *p_sai_recbuf1;     /* SAI 接收缓冲指针1 */
uint8_t *p_sai_recbuf2;     /* SAI 接收缓冲指针2 */

extern void (*sai1_rx_callback)(void);
extern DMA_HandleTypeDef g_sai1_tx_dma_handle;
/**
 * REC录音FIFO管理参数.
...
uint8_t *p_sai_recfifo_buf[REC_SAI_RX_FIFO_SIZE];   /* 录音接收FIFO数组指针 */
...
{
    if (g_sai_recfifo_rdpos == g_sai_recfifo_wrpos)     /* 读位置  =  写位置, 说明没数据可读 */
    {
        return 0;
    }
    ...
    if (g_sai_recfifo_rdpos >= REC_SAI_RX_FIFO_SIZE)    /* 读位置超过了总FIFO数, 归零重新开始 */
    {
...
uint8_t recoder_sai_fifo_write(uint8_t *buf)
{
    ...
    if (g_sai_recfifo_wrpos >= REC_SAI_RX_FIFO_SIZE)    /* 写位置超过了总FIFO数, 归零重新开始 */
    {
    ...
    for (i = 0; i < REC_SAI_RX_DMA_BUF_SIZE; i++)       /* 循环写数据 */
    {
...
void recoder_sai_dma_rx_callback(void)
{
    if (g_rec_sta == 0x80)                          /* 录音模式 */
    {
        ...
        {
            recoder_sai_fifo_write(p_sai_recbuf1);  /* p_sai_recbuf1 写入FIFO */
        }
        else
        {
            recoder_sai_fifo_write(p_sai_recbuf2);  /* p_sai_recbuf2 写入FIFO */
        }
    }
}

const uint16_t SAI_PLAY_BUF[2] = {0x0000, 0x0000};  /* 2个16位数据,用于录音时SAI Master发送.循环发送0. */
...
{
    SAI1_TX_DMASx->CR &= ~(1 << 4); /* 关闭传输完成中断(这里不用中断送数据) （如果在这里不关闭dma就会卡在清空数据过程）*/
    ...
    sai1_saia_init(SAI_MODEMASTER_TX, SAI_CLOCKSTROBING_RISINGEDGE, SAI_DATASIZE_16);   /* SAI1 Block A,主发送,16位数据 */
    sai1_saib_init(SAI_MODESLAVE_RX, SAI_CLOCKSTROBING_RISINGEDGE, SAI_DATASIZE_16);    /* SAI1 Block B从模式接收,16位 */

    sai1_samplerate_set(REC_SAMPLERATE);                                                /* 设置采样率 */

    sai1_tx_dma_init((uint8_t *)&SAI_PLAY_BUF[0], (uint8_t *)&SAI_PLAY_BUF[1], 1, 1);   /* 配置TX DMA,16位 */
    SAI1_TX_DMASx->CR &= ~(1 << 4);
    sai1_rx_dma_init(p_sai_recbuf1, p_sai_recbuf2, REC_SAI_RX_DMA_BUF_SIZE / 2, 1);     /* 配置RX DMA */

    sai1_rx_callback = recoder_sai_dma_rx_callback;     /* 初始化回调函数指sai_rx_callback */
...
void wav_recorder(void)
{
    ...
    {
        p_sai_recfifo_buf[i] = mymalloc(SRAMIN, REC_SAI_RX_DMA_BUF_SIZE); /* SAI录音FIFO内存申请 */
    ...
    p_sai_recbuf1 = mymalloc(SRAMIN, REC_SAI_RX_DMA_BUF_SIZE);          /* SAI录音内存1申请 */
    p_sai_recbuf2 = mymalloc(SRAMIN, REC_SAI_RX_DMA_BUF_SIZE);          /* SAI录音内存2申请 */
    ...
    if (rval == 0)
    {
        ...
        while (rval == 0)
        {
            ...
            if (recoder_sai_fifo_read(&pdatabuf))   /* 读取一次数据, 读到数据了, 写入文件 */
            {
                res = f_write(f_rec, pdatabuf, REC_SAI_RX_DMA_BUF_SIZE, &bw); /* 写入文件 */

...
void HAL_TIM_PWM_MspInit(TIM_HandleTypeDef *htim)
{
    if (htim->Instance == GTIM_TIMX_PWM)
    {
        ...
        gpio_init_struct.Pin = GTIM_TIMX_PWM_CHY_GPIO_PIN;              /* 通道y的GPIO */
        gpio_init_struct.Mode = GPIO_MODE_AF_PP;                        /* 复用推挽 */
        gpio_init_struct.Pull = GPIO_PULLUP;                            /* 上拉 */
        gpio_init_struct.Speed = GPIO_SPEED_FREQ_HIGH;                  /* 高速 */
        gpio_init_struct.Alternate = GTIM_TIMX_PWM_CHY_GPIO_AF;         /* GPIO复用选择 */
...
void gtim_timx_cnt_chy_init(uint16_t psc)
{
    ...
    HAL_NVIC_EnableIRQ(GTIM_TIMX_CNT_IRQn);                                /* 开启TIMx中断 */
...
{
    __HAL_TIM_DISABLE(&g_timx_cnt_chy_handle);                  /* 关闭定时器x */
    g_timxchy_cnt_ofcnt = 0;                                    /* 累加器清零 */
    __HAL_TIM_SET_COUNTER(&g_timx_cnt_chy_handle, 0);           /* 计数器清零 */
    __HAL_TIM_ENABLE(&g_timx_cnt_chy_handle);                   /* 使能定时器x */
}

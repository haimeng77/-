...
volatile uint8_t g_avi_sai_playbuf;         /* 即将播放的音频帧缓冲编号 */
uint8_t *p_avi_sai_buf[AVI_AUDIO_BUF_NUM];  /* 音频缓冲帧 */
...
void audio_sai_dma_callback(void)
{
    ...
    if (g_avi_sai_playbuf >= AVI_AUDIO_BUF_NUM)   /* 限定范围 */
    {
        g_avi_sai_playbuf = 0;
    }
    ...
    {
        SAI1_TX_DMASx->M0AR = (uint32_t)p_avi_sai_buf[g_avi_sai_playbuf];  /* 指向下一个buf */
    }
    else
    {
        SAI1_TX_DMASx->M1AR = (uint32_t)p_avi_sai_buf[g_avi_sai_playbuf];  /* 指向下一个buf */
    }
}
...
uint8_t video_play_mjpeg(uint8_t *pname)
{
    ...
    {
        p_avi_sai_buf[i] = mymalloc(SRAMIN, AVI_AUDIO_BUF_SIZE);   /* 申请音频内存 */
        
        if (p_avi_sai_buf[i] == NULL)                              /* 申请失败, 直接退出 */
        {
            break;
        }
        
        memset(p_avi_sai_buf[i], 0, AVI_AUDIO_BUF_SIZE);            /* 数据清零 */
    }
    ...
    while (res == 0)
    {
        ...
        if (res == 0)
        {
            ...
            {
                printf("avi err:%d,width:%d\r\n", res, g_avix.Width);
                res = KEY0_PRES;
                break;
            }

            video_info_show(&g_avix);
            btim_tim7_int_init(g_avix.SecPerFrame / 100 - 1, 20000 - 1);    /* 10Khz计数频率,加1是100us */
            ...
            if (g_avix.SampleRate)                              /* 有音频信息,才初始化 */
            {
                ...
                sai1_saia_init(SAI_MODEMASTER_TX, SAI_CLOCKSTROBING_RISINGEDGE, SAI_DATASIZE_16);   /* 设置SAI,主发送,16位数据 */
                sai1_samplerate_set(g_avix.SampleRate);         /* 设置采样率 */
                sai1_tx_dma_init(p_avi_sai_buf[0], p_avi_sai_buf[1], g_avix.AudioBufSize / 2, 1);   /* TX DMA 16位宽 */
                sai1_tx_callback = audio_sai_dma_callback;      /* 回调函数指向 audio_sai_dma_callback */
            ...
            while (1)               /* 播放循环 */
            {
                if (g_avix.StreamID == AVI_VIDS_FLAG)           /* 视频流 */
                {
                    ...
                    while (g_avi_frameup == 0);                 /* 待时间到达(在TIM的中断里面设置为1) */
                ...
                else                /* 音频流 */
                {
                    ...
                    f_read(favi, p_avi_sai_buf[saisavebuf], g_avix.StreamSize + 8, &nr); /* 填充p_avi_sai_buf */

 ...
 *
 *              高级定时器的时钟来自APB2,当PPRE2≥2分频的时候
 *              高级定时器的时钟为APB2时钟的2倍, 而APB2为100 M, 所以定时器时钟 = 200 Mhz
 *              定时器溢出时间计算方法: Tout = ((arr + 1) * (psc + 1)) / Ft us.
 *              Ft=定时器工作频率,单位:Mhz
 *
 * @param       arr: 自动重装值。
...
void atim_timx_cplm_pwm_init(uint16_t arr, uint16_t psc)
{
    ...
    ATIM_TIMX_CPLM->CR1 |= 2 << 8;      /* CKD[1:0] = 10, tDTS = 4 * tCK_INT = Ft / 4 = 50Mhz*/
 ...
 *              通用定时器的时钟为APB2时钟的2倍, 而APB2为100M, 所以定时器时钟 = 200Mhz
 *              定时器溢出时间计算方法: Tout = ((arr + 1) * (psc + 1)) / Ft us.
 *              Ft=定时器工作频率,单位:Mhz
 *
 *              本函数初始化的时候: 使用psc=0, arr固定为65535. 得到采样时钟频率为216Mhz,精度4.63ns
 *
 * @param       无
 * @retval      无
 */

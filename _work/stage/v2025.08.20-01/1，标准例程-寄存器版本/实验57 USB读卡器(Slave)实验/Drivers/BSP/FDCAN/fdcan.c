...
void HAL_FDCAN_MspInit(FDCAN_HandleTypeDef *hcan)
{
    if (FDCAN1 == hcan->Instance)
    {
        ...
        sys_gpio_set(FDCAN_RX_GPIO_PORT, FDCAN_RX_GPIO_PIN,
                     SYS_GPIO_MODE_AF, SYS_GPIO_OTYPE_PP, SYS_GPIO_SPEED_HIGH, SYS_GPIO_PUPD_PU);   /* FDCAN_TX引脚模式设置 */
        sys_gpio_set(FDCAN_TX_GPIO_PORT, FDCAN_TX_GPIO_PIN,
                     SYS_GPIO_MODE_AF, SYS_GPIO_OTYPE_PP, SYS_GPIO_SPEED_HIGH, SYS_GPIO_PUPD_PU);   /* FDCAN_TX引脚模式设置 */

        sys_gpio_af_set(FDCAN_RX_GPIO_PORT, FDCAN_RX_GPIO_PIN, 9);       /* FDCAN_RX脚, AF9 */
        sys_gpio_af_set(FDCAN_TX_GPIO_PORT, FDCAN_TX_GPIO_PIN, 9);       /* FDCAN_TX脚, AF9 */


        RCC->D2CCIP1R &= (uint32_t)3 << 28;             /* FDCANSEL[1:0]清零 */
        RCC->D2CCIP1R |= (uint32_t)1 << 28;             /* FDCANSEL[1:0]=1,选择pll1_q_ck作为FDCAN时钟源,一般为200Mhz */

#if FDCAN1_RX0_INT_ENABLE
        sys_nvic_init(1, 0, FDCAN1_IT0_IRQn, 2); //组2
#endif
    }
}
...
uint8_t fdcan_send_msg(uint8_t *msg, uint32_t len)
{
    ...
    g_fdcanx_txheade.TxEventFifoControl = FDCAN_NO_TX_EVENTS;        /* 无法送事件 */
